...
from model import ActiveCallReport
...
def str2DataFrame(data):
    ...
    df.rename(columns={'ID DA LIGAÇÃO': 'id_ligacao', 'DATA': 'data', 'TEMPO': 'hora', 'CAMPANHA': 'campanha', 'TIPO DE LIGAÇÃO': 'tipo_ligacao',
                       'AGENTE': 'agente', 'POSIÇÃO': 'posicao', 'NOME DO CLIENTE': 'nome_cliente', 'DNIS': 'dnis', 'TEMPO DE CONVERSA': 'tempo_conversa',
                       'TRANSFERÊNCIAS': 'transferencias', 'ABANDONADA': 'abandonada', 'Contrato': 'contrato', 'CPF-CNPJ': 'cpf_cnpj'},
                       inplace=True)
...
tableName, session_db, engine_db = ActiveCallReport.start(connString=p)
...
reportName = 'Report Voz - Ligacoes Ativas'
...
activeCallReport = str2DataFrame(data=result)

# Call of the function loadData for insert data on database
loadData(tableName=tableName, dataFrame=activeCallReport, session=session_db, engine=engine_db)