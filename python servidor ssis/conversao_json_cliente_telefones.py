...
sql_query = """SELECT [id_cliente], [telefones]
  FROM [sydle].[stg_cliente] where telefones is not null and telefones <> '[]'"""

source_df = pd.read_sql(sql_query, engine)
# print(source_df)
salvar = []

for id_cliente, telefones_str in source_df.values.tolist():
    dados = json.loads(telefones_str)
    for telefones in dados:
...
#
df.to_sql(name='stg_cliente_telefones', con=engine, if_exists='append', index=False, schema='sydle')
#
