...
sql_query = """SELECT [id_cliente], [emails]
  FROM [sydle].[stg_cliente] where emails is not null and emails <> '[]'"""

source_df = pd.read_sql(sql_query, engine)
# print(source_df)
salvar = []

for id_cliente, emails_str in source_df.values.tolist():
    dados = json.loads(emails_str)
    for emails in dados:
...
#
df.to_sql(name='stg_cliente_emails', con=engine, if_exists='append', index=False, schema='sydle')
#
