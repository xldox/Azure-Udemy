...
sql_query = """SELECT [id_cliente], [redes_sociais]
  FROM [sydle].[stg_cliente] where redes_sociais is not null and redes_sociais <> '[]'"""

source_df = pd.read_sql(sql_query, engine)
# print(source_df)
salvar = []

for id_cliente, redes_sociais_str in source_df.values.tolist():
    dados = json.loads(redes_sociais_str)
    for redes_sociais in dados:
        novo: dict = {'id_cliente': id_cliente,
                      'nome': redes_sociais['nome'],
...
#
df.to_sql(name='stg_cliente_redes_sociais', con=engine, if_exists='append', index=False, schema='sydle')
#
